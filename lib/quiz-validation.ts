...
export function createImportedQuizSchema(t: Translate) {
  ...
  const importedQuestionSchema = z
    .object({
      question: nonEmpty,
    ...
    .superRefine((value, ctx) => {
      if (new Set(value.choices).size !== value.choices.length)
        ctx.addIssue({ code: "custom", path: ["choices"], message: t("choicesDuplicate") });
      if (!value.choices.includes(value.correctAnswer))
        ctx.addIssue({
          code: "custom",
          path: ["correctAnswer"],
          message: t("correctAnswerMustBeChoice"),
        });
    });
    ...
    if (error instanceof z.ZodError)
      return {
        ...
        errors: error.issues.map((issue) =>
          t("issueMessage", { path: issue.path.join("."), message: issue.message }),
        ),
      };
